...
This kind of loops allow us to read in files into our loop, i.e. we might have two samples and want to do all comparisons. 
I.e. the comparisons we want tmake are are stored in a text file like this:
...
wdir="/export/lv3/scratch/bioinformatics_workshop/SXX_Assembly"
...
- go into that folder
- set a symbolic link to a folder with the metagenomic data, which is found here: `/export/lv3/scratch/bioinformatics_workshop/SXX_Assembly/rawdata_1/`
- set a symbolic link to a folder with secondary data we need from here: `/export/lv3/scratch/bioinformatics_workshop/SXX_Assembly/adaptors`
...
#prepare a folder for the assembly workflow
mkdir Assemblies
cd Assemblies

#create symbolic links to the metagenome we work with
ln -s /export/lv3/scratch/bioinformatics_workshop/SXX_Assembly/rawdata_1/ .

#create symbolic link to an adaptor file we need to clean up the genomes
ln -s /export/lv3/scratch/bioinformatics_workshop/SXX_Assembly/adaptors/ .

#create symbolic link to a folder with custom scripts
ln -s /export/lv3/scratch/bioinformatics_workshop/SXX_Assembly/scripts/ .
```
...
- The R1 and R2 files have the same nr of sequences. **This is important**, if you ever see that you don't have the same number this indicates that there might have been a problem when downloading the data or that the sequence center  might have done some cleaning. You should follow up on this and figure out why the numbers are not the same.
- You also see that sample NIOZ114 was sequenced much deeper than the other samples which might be important to keep in mind when interpreting the data.
...
- Overrepresented species. Here, esp. if we have adapter sequences left, you will see a sequence information. This is important to note down, as we want to remove adapters in the cleaning step and its important to check if we used the right adaptor information for the cleaning.
...
- run fastqc and multiqc on the fastq files generated with trimmomatic. Specifically, compare how the raw reads and trimmed reads compare.
...
[MEGAHIT](https://github.com/voutcn/megahit) is an ultra-fast and memory-efficient NGS assembler. It is optimized for metagenomes, but also works well on generic single genome assembly (small or mammalian size) and single-cell assembly. [Li et al., 2016](https://pubmed.ncbi.nlm.nih.gov/27012178/) The benefit over the other tool we use is that it works well while not using a lot of memory, which allows us to run it on smaller computers.
...
- We are only using two threads to run this job (via `-t 2`). For your own purposes increase the number to whatever is suitable for your cluster.
...
#lets control what we got --> should be +/- 6000 contigs
...
for sample in `cat SampleList`; do spades.py --meta -t 30 -k 21,39,59,99,127 -1  3_trimmomatic/${sample}_R1_paired.fastq.gz -2 3_trimmomatic/${sample}_R2_paired.fastq.gz -o 4_assemblies/metaspades/${sample}; done
```
...
5. Generally, contigs less than 2500 bp are not too informative and are often misbinned (if you want to assemble these contigs into MAGs), so for now we remove them and will not work with them
...
#check how many contigs we have now --> should be around 705
...
The summary statistics for the nr of contigs > 2500 bp and total sequence length should look something like this:
...
The files we get have so many contigs (this number needs to be consistent with the numbers we have above. Again, if we just work with NIOZ 1114, you might get some error messages that you can ignore):
...
Now we can clean things up to not store large/unneccessary files
...
We have a total of 1245 contigs from our 3 metagenomes with a length above 2,500 bp.
If we just worked with NIOZ114 we got 705 contigs, with a length above 2,500 bp.
