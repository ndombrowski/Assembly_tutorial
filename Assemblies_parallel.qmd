...
Contigs:<br/>
  ...
  If we check the N50, both megahit and metaspades give good scores. However with N75 this looks better for metaspades. What tool we decide on might also depend on the available memory, where megahit usually is better.]{.spoiler}
...
parallel -j3 'i={}; perl scripts/perl//length+GC.pl 4_assemblies/megahit/${i}/${i}_temp > 4_assemblies/megahit/${i}/${i}_length_gc.txt' ::: `cat SampleList`
...
7. now lets remove the short contigs from our actual sequences
...
8. Now we can clean things up to not store large/unneccessary files
...
If we just worked with NIOZ114 we got 359 contigs, with a length above 2,500 bp.
